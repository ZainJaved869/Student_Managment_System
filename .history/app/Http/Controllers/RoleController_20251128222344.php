...
public function users($id)
{
    $role = Role::findOrFail($id);
    $users = User::where('role_id', $id)->get();
    $usersWithoutRole = User::withoutRole()->get();
...
public function assignUser(Request $request, $id)
{
    ...
    try {
        ...
        if ($user->role_id) {
            return redirect()->back()
                ->with('error', 'User already has a role assigned. Please remove them from their current role first.');
        }
...
public function removeUser($roleId, $userId)
{
    ...
    try {
        ...
        if ($user->role_id != $roleId) {
            return redirect()->back()
                ->with('error', 'User is not assigned to this role.');
        }
...
public function bulkAssignUsers(Request $request, $id)
{
    $role = Role::findOrFail($id);
    ...
    try {
        ...
        $message = $assignedCount > 0 
            ? "Successfully assigned {$assignedCount} users to role!"
            : "No users were assigned. They may already have roles.";
        ...
        DB::rollBack();
        return redirect()->back()
            ->with('error', 'Error assigning users to role: ' . $e->getMessage());
    }
}